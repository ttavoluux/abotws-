...
const handler = async (m, {conn, text, participants, isOwner, isAdmin, args}) => {
    ...
    if (!m.quoted && !text){
        ...
        if(numer === 1){
            ...
            await conn.sendMessage(m.chat, { text : text ? text : `@ *${await conn.getName(m.chat)}*`+ tag, mentions: users}, { quoted: m, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
        }
        return
    }
